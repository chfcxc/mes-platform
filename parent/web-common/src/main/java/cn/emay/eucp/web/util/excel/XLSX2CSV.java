...
/**
 * A rudimentary XLSX -> CSV processor modeled on the POI sample program XLS2CSVmra by Nick Burch from the package org.apache.poi.hssf.eventusermodel.examples. Unlike the HSSF version, this one
 * completely ignores missing rows.
 * <p/>
 * Data sheets are read using a SAX parser to keep the memory footprint relatively small, so this should be able to read enormous workbooks. The styles table and the shared-string table must be kept
 * in memory. The standard POI styles table class is used, but a custom (read-only) class is used for the shared string table because the standard POI SharedStringsTable grows very quickly with the
 * number of unique strings.
 * <p/>
 * Thanks to Eric Smith for a patch that fixes a problem triggered by cells with multiple "t" elements, which is how Excel represents different formats (e.g., one word plain and one word bold).
 * 
 * @author Chris Lott
 */
public class XLSX2CSV {
			...
			if ("v".equals(name)) {
				...
				if (isOnlyReadFirstRow) {
					if (stringList.size() > 0) {
						throw new OnlyReadFirstRowWithXlsxException("");
					}
				}
	...
	public void process() throws IOException, OpenXML4JException, ParserConfigurationException, SAXException {
		...
		} catch (OnlyReadFirstRowWithXlsxException e) {
			if (e instanceof OnlyReadFirstRowWithXlsxException) {
				return;
			}
			throw e;
		} finally {
	 ...
	 * @return List<String[]>
	 */
	public List<String[]> readExcel07Bycsv(File file) throws Exception {
	...
	public static void main(String[] args) {
		File xlsxFile = new File("C:/Users/emay/Desktop/2.xlsx");
		String[] a = null;
		try {
			a = OnlyReadFirstRowUtil.getFirstRowFromXlsx(xlsxFile);
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println(JsonHelper.toJsonString(a));
		System.out.println(xlsxFile.delete());
	}
}