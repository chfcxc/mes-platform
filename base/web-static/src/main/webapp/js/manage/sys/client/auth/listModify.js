					...
					success:function(data){	
						...
						for(var i in ngvs){ 
							...
							for(var j in pagesNew){ //一级菜单
								...
								html += "<td class='roleNav roleLeft'  rowspan='"+pageLength+"' _pcode='"+pageCodeSecond+"'>"+pageNameSecond+"</label></td>";
				        ...
				        html+='<button type="button" class="tipBtn" id="saveBtn" onclick="ajaxmodify()">保存</button>';
						html +='</div>';
...
function ajaxmodify(){
	...
	if(authName.length>50){
		$.fn.tipAlert('权限名称不能大于50',1.5,0);
		return false;
	}
	...
	$.fn.tipLodding();
	$.ajax({
		url:  WEB_SERVER_PATH + '/sys/client/auth/ajax/update',
		type : 'post',
		dataType : 'json',
		data : {
			id : id,
		...
		success : function(data) {
			if (data.success) {
				...
				$.fn.tipAlert('客户权限保存成功',1.5,1);
				goPage('/sys/client/auth');
			} else {
