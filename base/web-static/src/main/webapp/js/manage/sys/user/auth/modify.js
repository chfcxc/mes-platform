...
$(function(){
	$.ajax({
		url:  WEB_SERVER_PATH + "/sys/auth/role/ajax/roleauth",
		type:'post',
		dataType:'json',
		data : {
			roleId :uid
		...
		success:function(data){	
			if(data.success){
				...
				var roleTypeCn = getSystemName(dataResult.roleType);
				var arr=[];
				...
				$.ajax({
					...
					success:function(data){	
						...
						html +='<dt class="item itemNew"><label class="item-label"><span class="xing">*</span>角色名称 :</label><input type="text" name="modifyroleName" id="modifyroleName" value="'+roleName+'" class="item-text" maxlength="20"  /></dt><div class="clear"></div>';
						html +='<dt class="item"><label class="item-label">所属系统 :</label>&nbsp;&nbsp;<span >'+roleTypeCn+'</dt></span ><div class="clear"></div>';
...
function ajaxmodify(){
	...
	var validateor= $formCheckOut.validate({
		...
		submitHandler: function() {
			...
			$.fn.tipLodding();
        	$.ajax({
        		url:  WEB_SERVER_PATH + '/sys/auth/role/ajax/modify?randomdata='+Date.parse(new Date()),
        		type : 'post',
        		dataType : 'json',
        		data : {
        			auths : tmp,
