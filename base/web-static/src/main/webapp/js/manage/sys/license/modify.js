//短彩信回显
$(function(){
	var serviceTypeVal=$("#serviceType input[type='hidden']").val();
	var arr=serviceTypeVal.split(",");
		$("#serviceType input[type='checkbox']").each(function(j,ele){
			var checkedVal=$(ele).val();
			if($.inArray(checkedVal,arr)!=-1){
	...
	var validateor=$formCheckOut.validate({
   	rules:{
		...
		submitHandler: function() {
			...
			$.fn.tipLodding();
        	$.ajax({
        		url : WEB_SERVER_PATH + '/sys/license/recode/ajax/update',
        		type : 'post',
        		dataType : 'json',
        		data : {
        			id : id,
