...
$.validator.addMethod("basesectionnumber", function(value, element, param){
    return  new RegExp(/^1[0-9]{2,4}$/).test(value); 
}, "请输入以1为开头的3到5位正整数");
...
$.validator.addMethod("real_name_new", function(value, element, param){
	if(value==''){
		return true;
	}
    return  new RegExp(/^[\u2E80-\u9FFFa-zA-Z]+$/).test(value); 
...
$.validator.addMethod("isnan", function(value, element, param){
	if(value==null||value==''){
		return true;
	}
...
//输入合法正数
$.validator.addMethod("postivezero", function(value, element, param){
	if(value==null||value==''){
		return true;
	}
	var number = +value;
...
//输入非负整数 可单独为0 不可以0开头
$.validator.addMethod("forwardRecitation", function(value, element, param){
	if(value==null||value==''){
		return true;
	}
    return  new RegExp(/^(0|[1-9]\d*)$/).test(value); 
}, "请输入非负整数");
$.validator.addMethod("numberMobiles", function(value, element, param){
	...
	if(value.length>11){
        ...
        for(var i=0; i < arr.length; i++){
        	if(arr[i].length>11){
        		 return  new RegExp(/^1[345789]\d{9}$/).test(arr[i]); 	
        	}	
        }
	}else{
		return  new RegExp(/^1[345789]\d{9}$/).test(value); 
	}
...
/*签名表*/
$.validator.addMethod("signExt2", function(value, element, param){
...
$.validator.addMethod("templateTestReg", function(value, element, param){
	if(value==null||value==''){
		return true;
	}
    return  new RegExp(/^[0-9]\d{0,6}$/).test(value); 
...
//请输入非负整数
$.validator.addMethod("nonNegative", function(value, element, param){
    return  new RegExp(/^\d*$/).test(value); 
}, "请输入非负整数");
//服务号校正 
...
/*新增服务号*/
$.validator.addMethod("servicenumbers", function(value, element, param){
	if(value==null||value==''){
		return true;
	}
	if(value.length > 32){
	...
	return  new RegExp(/^([\u4e00-\u9fa5a-zA-Z0-9]+)$/).test(value); 
	
}, "请输入除特殊字符，且小于32位的字符");
$.validator.addMethod("timerjg", function(value, element, param){
	return  new RegExp(/^(?!1000)[1-9]\d{3,}$/).test(value); 
}, "只能输入大于1000的正整数");
$.validator.addMethod("portnumber", function(value, element, param){
	return  new RegExp(/^(0|([1-9][0-9]?)|100)$/).test(value); 
}, "只能输入大于1025小于65534的数");

/*供应商邮箱*/
$.validator.addMethod("channelsupplierEmail", function(value, element, param){
    return  new RegExp(/^([a-zA-Z0-9]+[_|\_|\.|\-]?)*[\u4e00-\u9fa5a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.|\-]?)*[a-zA-Z0-9]*[\u4e00-\u9fa5a-zA-Z0-9]+\.[a-zA-Z]*[\u4e00-\u9fa5a-zA-Z0-9]{2,3}$/).test(value); 
}, "请输入正确的邮箱");
// 流量
$.validator.addMethod("phones", function(value, element, param){
	return  new RegExp(/^1[0-9]{10}$/).test(value); 
}, "请输入正确联系电话");
//联系人模块
$.validator.addMethod("groupNameExg", function(value, element, param){
	return  new RegExp(/^([\u4E00-\u9FA5]|\w)*$/).test(value); 
}, "组名不能包含特殊字符");//可扩展通道 
