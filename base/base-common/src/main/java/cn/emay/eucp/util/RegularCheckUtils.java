...
public class RegularCheckUtils {
	...
	/**
	 * 验证大于0小于等于65535的数字(端口号)
	 * 
	 * @param number
	 * @return
	 */
	public static Boolean checkNumLessThanNew65535(String number) {
		if (null == number || number.length() == 0 || "".equals(number)) {
			return false;
		}
		String regExp = "^([1-9]|[1-9]\\d|[1-9]\\d{2}|[1-9]\\d{3}|[1-5]\\d{4}|6[0-4]\\d{3}|65[0-4]\\d{2}|655[0-2]\\d|6553[0-5])$";
		Pattern p = Pattern.compile(regExp);
		Matcher m = p.matcher(number);
		return m.matches();
	}
	
	public static boolean isAppId(String s) {
		if (s!=null && !"".equals(s)) {
			if (s.length() == 19 && s.indexOf("-") == 4 && s.lastIndexOf("-") == 13) {
				return true;
			}
		}
		return false;
	 ...
	 * @return
	 */
	public static boolean isIncludeDoubleBytes(String content) {
	 ...
	 * @return
	 */
	public static boolean checkImsMobile(String mobile) {
		try {
