...
public class SendBatchFmsServlet {

	private static Logger logger = Logger.getLogger(SendBatchFmsServlet.class);
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!baseResult.getCode().equals(EucpInterFmsReponseCode.SUCCESS)) {
				...
				logger.info("[" + requestParams.getAppId() + "] send batch fms " + datajson + " remoteIP：" + remoteIp + " fail : " + baseResult.getLog());
				return;
			}
			// 业务参数校验
			CheckResult<String> fmsResult = check(requestParams.getAppId(), requestParams.getTemplateId(), requestParams.getFmses());
			if (!fmsResult.getCode().equals(EucpInterFmsReponseCode.SUCCESS)) {
				HttpUtils.responseResult(fmsResult.getCode(), null, response, null);
				logger.info("[" + requestParams.getAppId() + "] send batch fms " + datajson + " remoteIP：" + remoteIp + " fail : " + fmsResult.getLog());
				return;
			}
			// 发送
			SendFmsResponse[] fmsResponse = send(requestParams.getFmses(), requestParams.getTemplateId(), requestParams.getAppId(), remoteIp);
			// 响应
			HttpUtils.responseResult(EucpInterFmsReponseCode.SUCCESS, fmsResponse, response, null);
			logger.info("[" + requestParams.getAppId() + "] send batch fms " + datajson + "remoteIP：" + remoteIp + " success! spent time " + (System.currentTimeMillis() - start) + "ms");
		} catch (Exception e) {
