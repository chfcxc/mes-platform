 ...
 */
public abstract class AbstractReceiveAuditQueryReportHttpChannel extends AbstractChannel implements QueryReport, ReceiveAudit {

	private static Logger log = Logger.getLogger(AbstractReceiveAuditQueryReportHttpChannel.class);
	...
	public Result init(FmsChannel mmsChannel, Map<String, String> fmsChannelInfoMap) {
		...
		try {
			...
			log.error("通道Id：" + ChannelFrameWork.getInstance().getChannelId() + "推送审核报告启动失败", e);
			return Result.badResult("推送审核服务启动失败");
		}
		log.info("接收状态报告服务启动成功，端口为：" + fmsChannelInfoMap.get("jettyPort") + "推送审核报告地址为：" + fmsChannelInfoMap.get("auditUrl"));
