 ...
 */
public abstract class AbstractQueryAuditReceiveReportHttpChannel extends AbstractChannel implements ReceiveReport, QueryAudit {

	private static Logger log = Logger.getLogger(AbstractQueryAuditReceiveReportHttpChannel.class);
	...
	public Result init(FmsChannel mmsChannel, Map<String, String> fmsChannelInfoMap) {
		...
		try {
			...
			log.error("通道Id：" + ChannelFrameWork.getInstance().getChannelId() + "接收状态报告服务启动失败", e);
			return Result.badResult("接收状态报告服务启动失败");
		}
		log.info("接收状态报告服务启动成功，端口为：" + fmsChannelInfoMap.get("jettyPort") + "状态报告地址为：" + fmsChannelInfoMap.get("reportUrl"));
