				...
				<c:if test="${ moudle.resource.resourceCode == CURRENT_MOUDLE_CODE }">
					...
					<c:forEach items="${moudle.getChildren()}" var="nav">
						...
						<c:forEach items="${nav.getChildren()}" var="page">
							<li class=" <c:if test="${ page.resource.resourceCode == CURRENT_PAGE_CODE }">left_nav_high</c:if>">
								<i></i>
								<a href="${WEB_SERVER_PATH}${page.resource.resourcePath}" onclick="sessionStorage.clear()">${ page.resource.resourceName}</a>
							</li>
						</c:forEach>
						</ul>
					</c:forEach>
...
<div class="tipLoadding disno" id="loaddingTip"><div class="tc"><img src="${WEB_STATIC_PATH}/img/manage/loadding.gif"></div></div>