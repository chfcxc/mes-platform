...
function showItem(){
	...
	if (!flag) {
		...
		window['emTableConfig'].ajaxConfig.data.operator=$("#operator option:selected").val();
...
var emTableConfig = {
	...
	ajaxConfig : {
		url : WEB_SERVER_PATH + '/fms/message/detail/ajax/list',
		method : 'POST',
		data : {
	...
	tableConfig : {
		...
		rowItems : [{
			...
			title : "失败原因码",
			width : "6%",
			context : "@{channelReportDesc}"
			
		},{
			isShow : true,
			title : "失败原因",
			width : "6%",
			context : "@{channelReportStr}"
...
function chooseType(){
	...
	$.ajax({
		url : WEB_SERVER_PATH + '/fms/message/batch/ajax/getcontent',
		type : 'post',
		dataType : 'json',
		data : {
			saveType :saveType,
